...
def read_files_from_folder(folder_path, chunk_size=3000, overlap_size=200):
    all_chunks = []
    for filename in os.listdir(folder_path):
        if filename.endswith(".txt"):  # 只处理txt文件
            file_path = os.path.join(folder_path, filename)
            chunks = read_and_split_file(file_path, chunk_size, overlap_size)
...
chunk_size = 500
overlap_size = 20
all_chunks = read_files_from_folder(folder_path, chunk_size, overlap_size)
print("Total number of chunks:", len(all_chunks))

first_50_texts = all_chunks
# 输出结果
...
# 向向量数据库中添加文档
vector_db.batch_add_documents(first_50_texts)
